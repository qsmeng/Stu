...
public class Spider {
	...
	public static void main(String[] args) {
		aLinkFile = new File("G:/Spider/ALinks.txt");
		imgLinkFile = new File("G:/Spider/ImgLinks.txt");
		docLinkFile = new File("G:/Spider/DocLinks.txt");
		errorLinkFile = new File("G:/Spider/ErrorLinks.txt");
		// 用数组存储四个文件对象，方便进行相同操作
		File[] files = new File[] { aLinkFile, imgLinkFile, docLinkFile, errorLinkFile };
		try {
			File Spider = new File("G:/Spider");
			if (!Spider.exists()) {
				Spider.mkdirs(); // 如果文件不存在则创建文件夹
			}
			for (File file : files) {
